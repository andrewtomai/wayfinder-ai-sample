...
import type { AgentTool, Message, ToolCall, GenerateResponse } from "./types";
...
/**
 * Create a mock IAIClient with a controllable generate function.
 */
  ...
  return {
    generate: vi.fn(
      (
        ...
        tools: AgentTool[],
        systemInstruction: string,
      ) => Promise.resolve(responseFn(messages, tools, systemInstruction)),
    ),
  };
}
...
function createAgentConfig(overrides?: Partial<AgentConfig>): AgentConfig {
  return {
    client: createMockClient(() => ({ text: "", toolCalls: null })),
    tools: createMockTools(),
    buildSystemInstruction: () => "You are a test assistant.",
    maxIterations: 10,
    ...overrides,
  };
}
...
describe("Agent Context Preservation", () => {
  ...
  // Store a mutable response function reference
  let mockResponseFn:
    | ((context: {
        messages: Message[];
  ...
  beforeEach(() => {
    ...
    const client = createMockClient((messages) => {
      if (mockResponseFn) {
        const result = mockResponseFn({ messages });
        return {
    ...
    agent = new Agent(
      createAgentConfig({ client, tools }),
    );
  ...
  it("should preserve tool results in message history for subsequent iterations", async () => {
    ...
    const showPOITool = tools.find((t) => t.name === "showPOI")!;
    vi.mocked(searchTool.action).mockResolvedValue({
      results: [{ id: "bathroom-123", name: "Bathrooms", floor: 1 }],
    });
    vi.mocked(showPOITool.action).mockResolvedValue({ status: "displayed" });
  ...
  it("should properly format and persist tool calls and results as JSON messages", async () => {
    ...
    const searchTool = tools.find((t) => t.name === "search")!;
    vi.mocked(searchTool.action).mockResolvedValue({ results: [] });
    ...
    const toolResultsMessage = history.find(
      (msg) => msg.type === "tool_results",
    );
  ...
  it("should handle error results correctly without breaking context", async () => {
    ...
    mockResponseFn = () => {
      ...
      if (callCount === 1) {
        ...
        return {
          text: "The search tool failed, but I handled it gracefully.",
        };
      }
    };

    const searchTool = tools.find((t) => t.name === "search")!;
    vi.mocked(searchTool.action).mockRejectedValue(
  ...
  it("should count total tools called across all iterations", async () => {
    ...
    const searchTool = tools.find((t) => t.name === "search")!;
    const getPOIDetailsTool = tools.find((t) => t.name === "getPOIDetails")!;
    const showPOITool = tools.find((t) => t.name === "showPOI")!;
    vi.mocked(searchTool.action).mockResolvedValue({ id: "123" });
    vi.mocked(getPOIDetailsTool.action).mockResolvedValue({ details: "info" });
    vi.mocked(showPOITool.action).mockResolvedValue({ status: "shown" });
...
describe("Iteration Limit Awareness & Graceful Exhaustion", () => {
  ...
  it("should allow conversation continuation after exhaustion", async () => {
    ...
    mockResponseFn = (context: { messages: Message[] }) => {
      ...
      const lastContent =
        lastMessage?.type === "user_input" ||
        lastMessage?.type === "assistant_response"
    ...
    const firstContent =
      finalHistory[0]?.type === "user_input"
        ? (finalHistory[0].content as string)
        : "";
    const lastContent =
      finalHistory[finalHistory.length - 1]?.type === "assistant_response"

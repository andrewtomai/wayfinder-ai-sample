...
export interface AgentConfig {
  client: IAIClient;
  tools: AgentTool[];
  buildSystemInstruction: (iteration: number) => string;
...
export class Agent {
  ...
  private buildSystemInstruction: (iteration: number) => string;
  ...
  constructor(config: AgentConfig) {
    ...
    this.buildSystemInstruction = config.buildSystemInstruction;
...
Let me try again with that extra info—I'm usually pretty good at finding things!`;
  ...
  async chat(userMessage: string): Promise<AgentRunResult> {
    ...
    while (iterations < this.maxIterations) {
      ...
      const systemInstruction = this.buildSystemInstruction(iterations);

      // On the final iteration, don't provide tools - forces text response
      const toolsForThisIteration =
        iterations === this.maxIterations ? [] : this.tools;
     ...
     if (iterations >= this.maxIterations && !finalText) {
       logger.warn(`Hit max iterations (${this.maxIterations}), stopping loop`);
